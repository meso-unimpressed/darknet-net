...
void *video_out_in_thread(void *ptr)
{
...
void demo(char *cfgfile, char *weightfile, float thresh, int cam_index, const char *filename, char **names, int classes, int delay, char *prefix, int avg_frames, float hier, int w, int h, int frames, int fullscreen)
{
    ...
    if(writeVideo == 1){
        printf("Setting up video out.\n");
        // delete if file exists, important if user switches from named pipe to normal file
        // but keeps the file name the same
        unlink (videoPath);
        if (createNamedPipe){
            if(mkfifo(videoPath, 0755) == -1){
                printf("ERROR CREATING NAMED PIPE\n");
            };
        }
        videoWriter = cvCreateVideoWriter(videoPath, CV_FOURCC('M', 'J', 'P', 'G'), 8, cvSize(buff[0].w,buff[0].h), 1 );
    }
    int count = 0;
    if(!prefix && showWindow){
